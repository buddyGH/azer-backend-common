...
class UserRoleAudit(BaseModel):
    ...
    user_role = fields.ForeignKeyField(
        "models.UserRole", related_name="audit_logs", description="关联的用户角色记录", on_delete=fields.CASCADE
    )
    operation_type = fields.CharEnumField(UserRoleOperationType, description="操作类型")
